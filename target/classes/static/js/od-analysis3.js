(function($) {
	...
	$("#create-report")
			.on(
					...
					function() {
						$("#report-box").show();
						clearReport();
						writeReport("<h3>最近一周"+globalArea+"OD情况</h3><ol><li>堵塞情况良好</li><li>06:00-07:00为高峰时段，通过车辆80000辆次</li><li>琶洲卡口001为高峰卡口，通过车辆90000辆次</li><li>5.2%为非广州市车辆，呈上升趋势</li><ol>")
						drawReport(0, globalArea+"OD流向图",
								"");
						drawReport(1,globalArea+"OD时段图","高峰时段为08:00-08:15有50000辆次出发，09:00-09:07有60000辆次到达");
						drawReport(2,globalArea+"OD卡口图","高峰卡口为琶洲卡口004有50000辆次出发，琶洲卡口005有60000辆次到达");
						drawReport(3,globalArea+"OD归属地图","非广州市车牌共有5000个(25.0%)，其中佛山市车牌车牌最多有2500个(12.5%)");
						scrollTo("report-box");
	...
	$("#od-car-list table>tbody>tr").on('mouseover',function(){
		$(this).find('td').eq(1).addClass('text-danger');
		$(this).find('td').eq(1).find('i').addClass('fa-search').removeClass('fa-car');
	});
	$("#od-car-list table>tbody>tr").on('mouseout',function(){
		$(this).find('td').eq(1).removeClass('text-danger');
		$(this).find('td').eq(1).find('i').addClass('fa-car').removeClass('fa-search');
	});
	...
	function drawTime(id) {
		...
		option = {
			...
			xAxis : {
				type : 'category',
				boundaryGap : false,
				data : [ '0-3', '3-6', '6-9', '9-12', '12-15', '15-18',
						'18-21', '21-24' ]
			},
			yAxis : {
				type : 'value',
