(function ($) {
    ...
    //数据
    var pieData = [
        {value: 335, name: '琶洲卡口001'},
        {value: 310, name: '越秀公园卡口001'},
        {value: 234, name: '增城卡口001'},
        {value: 135, name: '白云卡口001'},
        {value: 1548, name: '其它'}
    ];
    var lineSeries=[
        {
    ...
    //类型列表点击事件
    $("#type-choose>.tab-pane>.control-sidebar-menu>li").on('click',function(){
        var choose=$(this).find(".control-sidebar-heading").html();
        var type=$(this).parent().parent().find(".control-sidebar-heading").html();
        $("#total-left").html('<div class="nav-tabs-custom">'+
            ...
            '</div>'+
            '</div>');
        $("#draw-chart0").css("height", parseInt($("#draw-chart0").width()) - 34);
        if(type=="概览"){
            $("#total-left").html('<div class="nav-tabs-custom">'+
                '<ul class="nav nav-tabs">'+
                '<li class="active"><a href="#tabs-to" data-toggle="tab" aria-expanded="true">周</a>'+
                ...
                '</div>');
            $("#draw-chart0").css("height", parseInt($("#draw-chart0").width()) - 34);
            totalMap("draw-chart0", choose, dataToSreies(mapData));
        }else{
            $("#total-left").html('<div class="nav-tabs-custom">'+
                '<ul class="nav nav-tabs">'+
                '<li class="active"><a href="#tabs-to" data-toggle="tab" aria-expanded="true">周</a>'+
    ...
    function resizeInit(){
        $("#draw-chart0").css("height", parseInt($("#draw-chart0").width()) - 34);
        $("#draw-chart1").css("height", $("#draw-chart1").width());
        $("#draw-chart2").css("height", $("#draw-chart2").width());
        $("#draw-chart3").css("height", $("#draw-chart3").width());
        $("#draw-chart4").css("height", $("#draw-chart4").width());
        $("#behaviour-analysis-line").css("height", parseInt($("#behaviour-analysis-line").width())/2);
        $("#behaviour-analysis-pie").css("height", $("#behaviour-analysis-pie").width());
        $("#behaviour-analysis-bar").css("height", $("#behaviour-analysis-bar").width());
    }
        ...
        option = {
            ...
            tooltip: {
                ...
                formatter: function (params) {
                    return params.data.originCrossName + "->" + params.data.destCrossName
                },
    ...
    function dataToSreies(data) {
        ...
        for (var i = 0; i < len; i++) {
            ...
            resultArr[i] = {
                ...
                data: [{
                    coords: [[data[i].origin_lng, data[i].origin_lat], [data[i].dest_lng, data[i].dest_lat]],
