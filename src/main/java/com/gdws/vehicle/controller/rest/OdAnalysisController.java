 ...
 */
@RestController
public class OdAnalysisController {
	@Autowired
	private Systemconfigs conf;

	@RequestMapping("odanalysis")
	public JSONObject odanalysis(String plateNo) {
		String url = "http://" + conf.getDomain() + ":" + conf.getPort();
		return JSONObject.parseObject(
				HttpClientUtils.doGet(url + "/crossOdAnalysis?plateNo=" + plateNo, null));
	}
}
